...
<div id="content-container" class="work-click-area" style="margin-top: 30vh;" bind:this={workContainer}>
  <div class="content-wrapper" on:mousedown|preventDefault={slider.onHold} on:mouseup={slider.onRelease} on:mouseleave={slider.onRelease} on:mousemove|preventDefault={slider.onMouseMove} bind:this={container} class:disabled={currentActive !== null}>
      ...
      <ul class="work-list" bind:this={listContainer} class:hold={isMouseDown}>
        ...
        {#await workItemsFetch then items}
          {#each items as item, i}
            <li class="list-item clickable passive" class:ambient={currentActive !== i && currentActive !== null} class:active={currentActive === i} bind:this={workItems[i]}>
              <div class="img-wrapper" use:workImageIntro={{ promise: inView, delay: 120 * i + 100 }}>
                {#await loadImage(`assets/imgs/work-back/${item.id}/cover.jpg`) then src}
                  <img bind:this={images[i]} {src} on:dragstart|preventDefault draggable="false" alt="{item.title} Background" />
                {/await}
              </div>
